...
#let tx_out(input, position, inputHeight, styles) = {
  ...
  let datum = if "datum" in input [
    *Datum:* \ 
    #v(-0.8em)
    #stack(dir: ttb, spacing: 0.4em,
      ..input.datum.pairs().map(((k,val)) => datum_field(1.0em, k, val))
    )
  ] else []
...
#let transaction(name, layer: "l1", inputs: (), outputs: (), signatures: (), certificates: (), validRange: none, notes: none) = style(styles => {
  ...
  let inputs = [
      ...
      #for input in inputs {
        ...
        if input.at("redeemer", default: none) != none {
          place(dx: start.x + 12.26em, dy: start.y - 1.3em)[#input.at("redeemer")]
        }
  ...
  let tx_color = if layer == "l1" { black } else { rgb("#1a872e") };
  let transaction = [
      #set align(center)
      #rect(
        stroke: tx_color,
